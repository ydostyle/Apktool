...
final public class AndrolibResources {
    ...
    private void aapt2Package(File apkFile, File manifest, File resDir, File rawDir, File assetDir, File[] include,
        ...
        if (buildOptions.hasAarPath) {
            ...
            cmd.add("--java");
            cmd.add(javaDir.getAbsolutePath());
            // use aar package name create R.java
            cmd.add("--custom-package");
            cmd.add(buildOptions.aarPackageName);
        } else if (rawDir != null) {
            cmd.add("-R");
    ...
    public void compileAarRes(File appDir, File aarDir) throws BrutException {
        String aaptPath = buildOptions.aaptPath;
        String aaptCommand = AaptManager.getAaptExecutionCommand(aaptPath, AaptManager.getAapt2());
        List<String> cmd = new ArrayList<>();
        cmd.add(aaptCommand);
        cmd.add("compile");
        cmd.add("--dir");
        File aarResDir = new File(aarDir, "res");
        cmd.add(aarResDir.getAbsolutePath());
